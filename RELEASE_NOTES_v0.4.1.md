...
| 资产 | 大小 | 说明 |
|------|------|------|
| `DeskPilot-Mcp-v0.4.0-win-x64.zip` | ~6.67 MB | MCP server 单文件 exe（自包含 .NET 8 运行时） |
| `DeskPilot-App-v0.4.0-win-x64.zip` | ~2.44 MB | DeskPilot 主程序（需要 .NET 8 桌面运行时） |
| `*.sha256` 校验和 | 99 B | SHA256 校验 |

### 新功能

#### 预编译 MCP Server
- `dotnet publish -p:PublishSingleFile=true -p:SelfContained=true -p:PublishTrimmed=true` 产出 **6.67 MB 单文件 exe**
- 用户**无需安装 .NET 8**——双击即跑
- 内置压缩 + 自解压 + trim 优化
